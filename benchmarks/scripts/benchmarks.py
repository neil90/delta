...
class Benchmark:
    ...
    """
    def __init__(self, benchmark_name, benchmark_spec, use_spark_shell, 
            serverless=False, local_delta_dir=None, cloud_provider=None, benchmark_path=None):
        now = datetime.now()
    ...
    def serverless_dataproc_run(self,):
        client = dataproc_v1.BatchControllerClient()

        # Initialize request argument(s)
        batch = dataproc_v1.Batch()
        #batch.pyspark_batch.main_python_file_uri = "main_python_file_uri_value"
        batch.spark_batch.main_class = self.benchmark_spec['benchmark_main_class']
        batch.spark_batch.jar_file_uris = self.upload_jar_to_cloud_storage()
        batch.spark_batch.args = self.benchmark_spec['benchmark_main_class_args']
        batch.spark_batch.args.append("--conf")
        batch.spark_batch.args.append(f"spark.benchmarkId={self.benchmark_id}")
...
""".strip()
    ...
    def upload_jar_to_cloud_storage(self):
        # Compile JAR
        # Note: Deleting existing JARs instead of sbt clean is faster
        if os.path.exists("target"):
            run_cmd("""find target -name "*.jar" -type f -delete""", stream_output=True)
        ...
        if self.cloud_provider == 'gcp':
            jar_cloud_path = f"gs://{self.benchmark_path}/jars/{self.benchmark_id}-benchmarks.jar"
            run_cmd(f"gsutil cp {jar_local_path} {jar_cloud_path}")
            return jar_cloud_path
